...
pub async fn cmd_lyrics_get_for_track(
    ...
    if let Some(embedded) = read_embedded_lyrics(&track.path) {
        return Ok(resolve_precedence(request.track_id, Some(embedded), None, None));
    }

    if let Some(cached) = load_cached_lyrics(&conn, request.track_id)? {
        return Ok(resolve_precedence(request.track_id, None, Some(cached), None));
    }

    let track_for_lookup = track.clone();
    let fetched = tauri::async_runtime::spawn_blocking(move || fetch_lrclib_lyrics(&track_for_lookup))
        .await
...
fn load_track_lookup(
    ...
    let row: Option<(String, Option<String>, Option<String>, Option<String>, Option<i64>)> = conn
        .query_row(
    ...
    Ok(row.map(
        |(path, title, artist, album, duration_ms)| TrackLookup {
            path,
...
fn load_cached_lyrics(
    ...
    let cached: Option<(Option<String>, Option<String>)> = conn
        .query_row(
            "SELECT synced_lyrics, plain_lyrics FROM lyrics_cache WHERE track_id = ?1",
            rusqlite::params![track_id],
    ...
    Ok(cached.and_then(|(synced_lyrics, plain_lyrics)| {
        let resolved = ResolvedLyrics {
...
fn score_candidate(candidate: &LrclibCandidate, track: &TrackLookup) -> i32 {
    ...
    let normalized_candidate_title = normalize_string(candidate.track_name.as_deref().unwrap_or(""));
    let normalized_candidate_artist = normalize_string(candidate.artist_name.as_deref().unwrap_or(""));
    ...
    if let (Some(track_duration), Some(candidate_duration)) =
        (track.duration_secs, duration_secs_from_candidate(candidate.duration))
...
mod tests {
    ...
    fn test_precedence_embedded_first() {
        ...
        assert_eq!(response.synced_lyrics.as_deref(), Some("[00:01.00]embedded"));
